...
module.exports = function setupMessageHandler(sock, customStore, handleMessage, smsg, io, config) {
    ...
    // Anti Call with advanced scheduling & per‑contact control
    // Whitelist / Blacklist ALWAYS active – Global only controls unknown callers
    sock.ev.on('call', async (calls) => {
        try {
            ...
            const ownerNumber = process.env.OWNER_NUMBER || config.owner || '';
            const ownerJid = ownerNumber ? `${ownerNumber.replace(/\D/g, '')}@s.whatsapp.net` : null;
            ...
            for (const call of calls) {
                ...
                if (isInWhitelist(normalizedCaller, config.whitelist)) {
                    console.log(`[ANTICALL] Whitelisted — allowing call`);
                    continue;
                }

                // 2. BLACKLIST – ALWAYS REJECT (overrides global)
                ...
                // 3. Pending phone → LID upgrade (when a previously added number calls)
                ...
                // 4. Global OFF → allow unknown callers
                if (!config.enabled) {
                    console.log(`[ANTICALL] Global OFF — allowing unknown caller`);
                    continue;
                }

                // 5. Schedule check (only for unknown callers when global ON)
                if (!isWithinSchedule(config.schedule)) {
                    console.log(`[ANTICALL] Outside schedule — ignoring unknown caller`);
                    continue;
                }

                // 6. Reject unknown caller
                const reasonToSend = config.unknownReason || config.reason;
                await sock.sendMessage(callerJid, { text: reasonToSend }).catch(() => {});
                if (typeof sock.rejectCall === 'function') {
                    await sock.rejectCall(call.id, call.from).catch(() => {});
                }

                // DM owner with LID for easy whitelist/block
                if (ownerJid) {
                ...
                console.log(`[ANTICALL] Unknown caller rejected — JID: ${normalizedCaller}`);
            }
        } catch (err) {
