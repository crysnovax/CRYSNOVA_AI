...
║   CRYSN⚉VA AI V2.0 - Professional        ║

║   CONNECTION : CRYSN⚉VA V1.0 Style       ║

║   MODEL      : CRYSN⚉VA-Ai Style         

║ ✪ PLEASE RESTART AFTER SUCCESSFULLY CONNECTED 
...
const clientstart = async () => {
    ...
    // ── Interactive Setup & Pairing Code ────────────────────────
    ...
    if (config().status.terminal && !sock.authState.creds.registered) {
        ...
        let userConfig = null;
        let ownerName, finalBotName, finalPrefix, isPublic;
...
║   you will not be asked to do this again                                     
   ...
   3. Enter your PREFIX 
   4. ENTER YOUR MODE: Public/Private 
   
     Please don't forget to ⭐ my repo!
     Love you 😘       ║
╚════════════════════════════════════════════════════════╝
            `));
            ...
            ownerName = await question(chalk.cyan('👤 Enter your name (e.g., CRYSNOVA): '));
            ...
            // Question 2: Bot Name
            const botName = await question(chalk.cyan('\n🤖 Enter bot name (e.g., CRYSNOVA AI): '));
            finalBotName = botName.trim() || 'CRYSNOVA AI';

            // Question 3: Prefix
            console.log(chalk.gray('\n💡 Prefix is the symbol before commands (e.g., .menu or !menu)'));
            const prefix = await question(chalk.cyan('⚡ Enter command prefix (default is .): '));
            finalPrefix = prefix.trim() || '.';

            // Question 4: Public/Private Mode
            console.log(chalk.gray('\n💡 Public mode: Anyone can use the bot'));
            console.log(chalk.gray('   Private mode: Only you (owner) can use the bot'));
            const publicMode = await question(chalk.cyan('🔓 Public mode? (yes/no, default yes): '));
            isPublic = !publicMode || publicMode.toLowerCase() === 'yes' || publicMode.toLowerCase() === 'y';

            // Summary
            console.log(chalk.green.bold('\n\n✅ Configuration Summary:'));
            console.log(chalk.cyan(`👤 Owner Name:    ${chalk.white.bold(ownerName.trim())}`));
            console.log(chalk.cyan(`🤖 Bot Name:      ${chalk.white.bold(finalBotName)}`));
            console.log(chalk.cyan(`⚡ Prefix:        ${chalk.white.bold(finalPrefix)}`));
            console.log(chalk.cyan(`🔓 Mode:          ${chalk.white.bold(isPublic ? 'Public' : 'Private')}`));

            console.log(chalk.yellow('\n\n📱 Now let\'s connect your WhatsApp...\n'));
        }

        // Get WhatsApp number
        const phoneNumber = await question(chalk.cyan('📞 Enter your WhatsApp number (with country code, e.g., 2348077528901):\n') + chalk.white('Number: '));
        ...
        if (!fs.existsSync(USER_CONFIG_PATH)) {
            const setupConfig = {
                ...
                owner: {
                    ...
                    jid:    `${phoneNumber.trim()}@s.whatsapp.net`
                },
                bot: {
                    name:   finalBotName,
                    prefix: finalPrefix,
            ...
            try {
                const dbDir = path.dirname(USER_CONFIG_PATH);
                if (!fs.existsSync(dbDir)) fs.mkdirSync(dbDir, { recursive: true });
                fs.writeFileSync(USER_CONFIG_PATH, JSON.stringify(setupConfig, null, 2), 'utf8');
                console.log(chalk.green.bold('\n✅ Configuration saved!\n'));
            } catch (err) {
        ...
        console.log(chalk.gray('   Settings → Linked Devices → Link a Device → Link with phone number\n'));
