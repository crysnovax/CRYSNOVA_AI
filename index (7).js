...
try {
    const ghostPlugin = require('./src/Commands/Bot/ghost-mode.js');

    if (ghostPlugin.forceGhostPresence) {
        await ghostPlugin.forceGhostPresence(sock);
    }

} catch (e) {
    console.log('[GHOST MODE]', e.message);
}

const m = await smsg(sock, mek, store);
if (!m) return;

/* Invisible trigger handler */
...
/* Plugin capture system */

if (m.key?.fromMe) {

for (const plugin of Object.values(global.plugins || {})) {
if (typeof plugin.capture === 'function') {
await plugin.capture(sock, m, {
prefix: '.',
store
});
}
...
const mentionReactPlugin =
require('./src/Commands/Owner/mention-react');

await mentionReactPlugin?.handleMentionReaction?.(sock, m);
...
if (m.text && !m.key.fromMe && m.text.length >= 8) {

const isPrivate = !m.isGroup;

const isMentioned = m.mentionedJid?.some(j =>
sock.decodeJid(j) === sock.decodeJid(sock.user.id)
);

const isRepliedToBot =
m.quoted?.sender &&
sock.decodeJid(m.quoted.sender) === sock.decodeJid(sock.user.id);
