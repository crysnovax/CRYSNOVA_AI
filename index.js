...
const clientstart = async () => {
    ...
    for (const plugin of Object.values(global.plugins || {})) {
        if (typeof plugin.capture === 'function') {
            await plugin.capture(sock, m, { prefix: '.', store });
        }
    }
}
            ...
            if (m.body && m.key && !m.key.fromMe) {
        try {
        const mentionReactPlugin = require('./src/Commands/Owner/mention-react'); // adjust path if needed
        if (mentionReactPlugin.handleMentionReaction) {
            await mentionReactPlugin.handleMentionReaction(sock, m);
        }
    } catch (e) {
            console.error('[MENTION REACT]', e.message);
            }
            }
