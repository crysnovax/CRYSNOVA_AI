...
async function handlePluginCommands(sock, m, store) {
    ...
    // ────────────────────────────────────────────────
    // Get text from possible locations (very important!)
    // ────────────────────────────────────────────────
    let text = '';
    ...
    if (m.message?.conversation) {
        ...
        // If this is a reply message, we can still get text from quoted if needed
        text = m.text || '';
    }
    ...
    const commandPart = body.slice(1).trim();
    const command = commandPart.split(' ')[0].toLowerCase();
    const args = commandPart.slice(command.length).trim().split(' ').filter(Boolean);
    ...
    try {
        ...
        await sock.sendMessage(m.chat, { 
            text: `❌ Plugin Error: ${error.message || 'unknown'}` 
        }, { quoted: m });
    }
}
