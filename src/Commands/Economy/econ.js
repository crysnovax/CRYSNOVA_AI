...
// Helper: Send alert to BOT'S OWN DM (like ass.js sends to owner DM)
async function sendBotOwnerAlert(sock, alertData) {
    try {
        // Get the bot's own number from sock - EXACT same method as ass.js
        ...
        if (unread.length > 0) {
            ...
            // Send to BOT'S OWN DM - just like ass.js sends statuses to owner DM
...
// ==================== PAY (with alert to bot owner DM) ====================
cmds.push({
    ...
    execute: async (sock, m, { args, reply }) => {
        ...
        try {
            ...
            await sendTable(sock, m.chat,
                ...
                '💡 Alert sent to bot owner DM!'
            );
            await sock.sendMessage(m.chat, { react: { text: '🔖', key: m.key } });
            
            // 🔔 SEND ALERT TO RECIPIENT'S BOT OWNER DM
            try {
                const targetAlerts = await eco('GET /alerts', targetPhone);
                await sendBotOwnerAlert(sock, targetAlerts.data);
            } catch (e) {}
...
// ==================== ROB (with alert to victim's bot owner DM) ====================
cmds.push({
        ...
        try {
            ...
            if (d.success) {
                await sendTable(sock, m.chat,
                    ...
                    '💡 Alert sent to victim\'s bot owner DM!'
                );
                await sock.sendMessage(m.chat, { react: { text: '🔖', key: m.key } });
                
                // 🔔 SEND ALERT TO VICTIM'S BOT OWNER DM
                try {
                    const targetAlerts = await eco('GET /alerts', targetPhone);
                    await sendBotOwnerAlert(sock, targetAlerts.data);
                } catch (e) {}
        ...
        f: async (s, m, p, a) => {
            ...
            if (d.win) {
                try {
                    ...
                    await sendBotOwnerAlert(s, targetAlerts.data);
                } catch (e) {}
            }
        }
    },
