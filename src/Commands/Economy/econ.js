...
// Helper: Send DM alert to target
async function sendTargetDM(sock, targetPhone, alertData) {
    try {
        ...
        if (unread.length > 0) {
            ...
            console.log(`[ECON DM] Alert sent to ${targetPhone}`);
        }
    } catch (e) {
        console.log(`[ECON DM] Failed to DM ${targetPhone}:`, e.message);
    }
}
...
cmds.push({
    ...
    execute: async (sock, m, { args, reply }) => {
        ...
        try {
            ...
            // 🔔 AUTO DM THE TARGET THEIR ALERTS
            try {
    ...
    execute: async (sock, m, { args, reply }) => {
        ...
        try {
            ...
            if (d.success) {
                ...
                // 🔔 AUTO DM THE VICTIM THEIR ALERTS
                try {
                    const targetAlerts = await eco('GET /alerts', targetPhone);
                    await sendTargetDM(sock, targetPhone, targetAlerts.data);
                } catch (e) {}
