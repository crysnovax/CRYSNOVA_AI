...
console.log(chalk.cyan(`📦 Loaded ${pluginManager.list().length} plugins`));
// ===== END PLUGIN MANAGER =====
...
// --- Group Events Handler (Simplified, Profile Pic Only) ---
      ...
      for (let user of data.participants) {
        try {
          ...
          await sock.sendMessage(data.id, {
            ...
            caption: `👋 Welcome @${userId.split('@')[0]}!\n` +
                     `👥 Members: ${memberCount}\n` +
    ...
    if (data.action === 'remove') {
      for (let user of data.participants) {
        try {
          ...
          await sock.sendMessage(data.id, {
            text: `👋 @${userId.split('@')[0]} left the group.\n\n${goodbyeText}`,
            mentions: [userId]
          });
        } catch (err) {
